...
import { Building, MapPin, MessageCircle, Check, Trash2, Loader2, ShieldCheck, Eye, EyeOff } from 'lucide-react';

const API_BASE = import.meta.env.VITE_API_URL || 'https://aparecidadonortesp.com.br';

const ADMIN_PASSWORD = 'admin123';

interface HotelPendente {
  id: string;
  nome: string;
...
const PlanoTag: React.FC<{ plano: string | null }> = ({ plano }) => {
  const styles: Record<string, string> = {
    ...
    destaque: 'bg-blue-100 text-blue-700 ring-1 ring-blue-300',
    ...
    <div className="min-h-screen bg-gray-50 flex items-center justify-center px-4">
      <div className="bg-white rounded-2xl shadow-sm border border-gray-200 p-8 w-full max-w-sm">
        <div className="flex flex-col items-center mb-8">
          ...
          <p className="text-sm text-gray-500 mt-1">Gerenciamento de hotéis</p>
        </div>
...
const HotelCard: React.FC<HotelCardProps> = ({ h, acaoId, getCidades, onAprovar, onRejeitar, onExcluir }) => (
  <div className="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden">
    <div className="flex flex-col sm:flex-row gap-0">
      ...
      <div className="sm:w-40 shrink-0 bg-gray-100">
        {h.foto_url ? (
          <img src={h.foto_url} alt={h.nome} className="w-full h-48 sm:h-full object-cover" />
        ) : (
      ...
      <div className="flex-1 p-5 flex flex-col justify-between gap-4">
        <div className="space-y-2">
          <div className="flex items-center gap-3 flex-wrap">
            <h2 className="text-lg font-semibold text-gray-900">{h.nome}</h2>
            <PlanoTag plano={h.plano} />
          </div>

          <div className="grid grid-cols-1 sm:grid-cols-2 gap-1.5 text-sm text-gray-600">
            <div className="flex items-center gap-2">
              <MessageCircle className="w-4 h-4 text-green-500 shrink-0" />
              <span>{h.whatsapp || h.email || '—'}</span>
            </div>
            <div className="flex items-center gap-2">
            ...
            <div className="flex items-start gap-2 sm:col-span-2">
              ...
              <span>{getCidades(h.cidades) || '—'}</span>
            </div>
          </div>
        ...
        <div className="flex items-center gap-3 pt-2 border-t border-gray-100 flex-wrap">
          {onAprovar && (
            <button
              onClick={() => onAprovar(h.id)}
              disabled={acaoId === h.id}
            ...
            >
              {acaoId === h.id ? <Loader2 className="w-4 h-4 animate-spin" /> : <Check className="w-4 h-4" />}
              Aprovar
            </button>
          )}
          {onRejeitar && (
            <button
              onClick={() => onRejeitar(h.id)}
              disabled={acaoId === h.id}
            ...
            >
              {acaoId === h.id ? <Loader2 className="w-4 h-4 animate-spin" /> : <Trash2 className="w-4 h-4" />}
              Rejeitar
            </button>
          )}
          {onExcluir && (
            <button
              onClick={() => onExcluir(h.id)}
              disabled={acaoId === h.id}
...
const AdminHotels: React.FC = () => {
  ...
  const [pendentes, setPendentes] = useState<HotelPendente[]>([]);
  const [aprovados, setAprovados] = useState<HotelPendente[]>([]);
  ...
  const fetchPendentes = async () => {
    setCarregando(true);
    try {
      const res = await fetch(`${API_BASE}/api/admin/hotels-pendentes`, {
        headers: { 'x-admin-password': ADMIN_PASSWORD },
      });
      const data = await res.json();
      if (res.ok && data.hotels) setPendentes(data.hotels as HotelPendente[]);
    } catch (e) {
  ...
  const fetchAprovados = async () => {
    setCarregando(true);
    try {
      const res = await fetch(`${API_BASE}/api/admin/hotels-ativos`, {
        headers: { 'x-admin-password': ADMIN_PASSWORD },
      });
      const data = await res.json();
      if (res.ok && data.hotels) setAprovados(data.hotels as HotelPendente[]);
    } catch (e) {
  ...
  const aprovar = async (id: string) => {
    setAcaoId(id);
    try {
      const res = await fetch(`${API_BASE}/api/admin/aprovar-hotels`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', 'x-admin-password': ADMIN_PASSWORD },
        body: JSON.stringify({ id }),
      });
      if (res.ok) {
        const hotel = pendentes.find(h => h.id === id);
        setPendentes(prev => prev.filter(h => h.id !== id));
        if (hotel) setAprovados(prev => [hotel, ...prev]);
      }
    } catch (e) {
  ...
  const rejeitar = async (id: string) => {
    ...
    setAcaoId(id);
    try {
      const res = await fetch(`${API_BASE}/api/admin/rejeitar-hotels`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', 'x-admin-password': ADMIN_PASSWORD },
        body: JSON.stringify({ id }),
      });
      if (res.ok) setPendentes(prev => prev.filter(h => h.id !== id));
    } catch (e) {
  ...
  const excluir = async (id: string) => {
    if (!window.confirm('Tem certeza que deseja excluir este hotel aprovado?')) return;
    setAcaoId(id);
    try {
      const res = await fetch(`${API_BASE}/api/admin/rejeitar-hotels`, {
        method: 'POST',
      ...
      if (res.ok) setAprovados(prev => prev.filter(h => h.id !== id));
    } catch (e) {
        ...
        {carregando ? (
          ...
          <div className="flex flex-col items-center justify-center py-24 text-gray-400">
            ...
            <p className="text-base font-medium text-gray-600">
              {aba === 'pendentes' ? 'Nenhum cadastro pendente' : 'Nenhum hotel aprovado'}
            </p>
          </div>
        ) : (
