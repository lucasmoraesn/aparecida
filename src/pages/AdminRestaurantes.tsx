...
import { UtensilsCrossed, MapPin, MessageCircle, Check, Trash2, Loader2, ShieldCheck, Eye, EyeOff } from 'lucide-react';

const API_BASE = import.meta.env.VITE_API_URL || 'https://aparecidadonortesp.com.br';

const ADMIN_PASSWORD = 'admin123';

interface RestaurantePendente {
  id: string;
  nome: string;
...
const PlanoTag: React.FC<{ plano: string | null }> = ({ plano }) => {
  const styles: Record<string, string> = {
    premium: 'bg-orange-100 text-orange-700 ring-1 ring-orange-300',
    destaque: 'bg-green-100 text-green-700 ring-1 ring-green-300',
    ...
    <div className="min-h-screen bg-gray-50 flex items-center justify-center px-4">
      <div className="bg-white rounded-2xl shadow-sm border border-gray-200 p-8 w-full max-w-sm">
        <div className="flex flex-col items-center mb-8">
          <div className="w-14 h-14 bg-green-600 rounded-2xl flex items-center justify-center mb-4 shadow-md">
            <ShieldCheck className="w-7 h-7 text-white" />
          </div>
          <h1 className="text-xl font-bold text-gray-900">Área Administrativa</h1>
          <p className="text-sm text-gray-500 mt-1">Gerenciamento de restaurantes</p>
        </div>
          ...
          <div>
            ...
            <div className="relative">
              <input
                ...
                className={`w-full px-4 py-2.5 pr-10 rounded-lg border text-sm focus:outline-none focus:ring-2 focus:ring-green-500 ${
...
const RestauranteCard: React.FC<RestauranteCardProps> = ({ r, acaoId, onAprovar, onRejeitar, onExcluir }) => (
  <div className="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden">
    <div className="flex flex-col sm:flex-row gap-0">
      ...
      <div className="sm:w-40 shrink-0 bg-gray-100">
        {r.foto_url ? (
          <img src={r.foto_url} alt={r.nome} className="w-full h-48 sm:h-full object-cover" />
        ) : (
      ...
      <div className="flex-1 p-5 flex flex-col justify-between gap-4">
        <div className="space-y-2">
          <div className="flex items-center gap-3 flex-wrap">
            <h2 className="text-lg font-semibold text-gray-900">{r.nome}</h2>
...
const AdminRestaurantes: React.FC = () => {
  const [autenticado, setAutenticado] = useState(false);
  const [aba, setAba] = useState<'pendentes' | 'aprovados'>('pendentes');
  const [pendentes, setPendentes] = useState<RestaurantePendente[]>([]);
  const [aprovados, setAprovados] = useState<RestaurantePendente[]>([]);
  ...
  const fetchPendentes = async () => {
    setCarregando(true);
    try {
      const res = await fetch(`${API_BASE}/api/admin/restaurantes-pendentes`, {
        headers: { 'x-admin-password': ADMIN_PASSWORD },
      });
      const data = await res.json();
      if (res.ok && data.restaurantes) setPendentes(data.restaurantes as RestaurantePendente[]);
    } catch (e) {
  ...
  const fetchAprovados = async () => {
    setCarregando(true);
    try {
      const res = await fetch(`${API_BASE}/api/admin/restaurantes-ativos`, {
        headers: { 'x-admin-password': ADMIN_PASSWORD },
      });
      const data = await res.json();
      if (res.ok && data.restaurantes) setAprovados(data.restaurantes as RestaurantePendente[]);
    } catch (e) {
  ...
  useEffect(() => {
    if (autenticado) {
      document.title = 'Admin — Restaurantes';
      fetchPendentes();
      fetchAprovados();
    }
  }, [autenticado]);
  ...
  const aprovar = async (id: string) => {
    setAcaoId(id);
    try {
      const res = await fetch(`${API_BASE}/api/admin/aprovar-restaurantes`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', 'x-admin-password': ADMIN_PASSWORD },
        body: JSON.stringify({ id }),
      });
      if (res.ok) {
        const restaurante = pendentes.find(r => r.id === id);
        setPendentes(prev => prev.filter(r => r.id !== id));
        if (restaurante) setAprovados(prev => [restaurante, ...prev]);
      }
    } catch (e) {
  ...
  const rejeitar = async (id: string) => {
    ...
    setAcaoId(id);
    try {
      const res = await fetch(`${API_BASE}/api/admin/rejeitar-restaurantes`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', 'x-admin-password': ADMIN_PASSWORD },
        body: JSON.stringify({ id }),
      });
      if (res.ok) setPendentes(prev => prev.filter(r => r.id !== id));
    } catch (e) {
  ...
  const excluir = async (id: string) => {
    if (!window.confirm('Tem certeza que deseja excluir este restaurante aprovado?')) return;
    setAcaoId(id);
    try {
      const res = await fetch(`${API_BASE}/api/admin/rejeitar-restaurantes`, {
        method: 'POST',
      ...
      if (res.ok) setAprovados(prev => prev.filter(r => r.id !== id));
    } catch (e) {
      ...
      <div className="max-w-5xl mx-auto">
        ...
        <div className="flex items-center justify-between mb-6">
          ...
          <button
            ...
            className="text-sm text-green-600 hover:text-green-800 font-medium flex items-center gap-1.5"
          >
        ...
        {carregando ? (
          ...
          <div className="flex flex-col items-center justify-center py-24 text-gray-400">
            ...
            <p className="text-base font-medium text-gray-600">
              {aba === 'pendentes' ? 'Nenhum cadastro pendente' : 'Nenhum restaurante aprovado'}
            </p>
          </div>
        ) : (
