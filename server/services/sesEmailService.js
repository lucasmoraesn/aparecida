...
// ─────────────────────────────────────────────────────────────────────────────
// 🏢 GENÉRICO: Hotels & Restaurantes (reutiliza template de motorista)
// ─────────────────────────────────────────────────────────────────────────────

/**
 * Notifica o administrador que um novo hotel/restaurante foi cadastrado.
 * Reutiliza o template de Motorista (icon/cores padronizadas).
 *
 * @param {Object} params
 * @param {string} params.type      - 'hotel' ou 'restaurante'
 * @param {string} params.nome      - Nome do estabelecimento
 * @param {string} params.whatsapp  - WhatsApp
 * @param {string} params.email     - E-mail do proprietário
 * @param {string} params.plano     - Plano contratado
 */
export async function sendNewBusinessNotification({ type, nome, whatsapp, email, plano }) {
    const adminEmail = process.env.ADMIN_EMAIL;
    if (!adminEmail) {
        console.warn(`⚠️  [SES] ADMIN_EMAIL não configurado — notificação de ${type} ignorada.`);
        return { success: false, error: 'ADMIN_EMAIL não configurado' };
    }
    ...
    const typeLabel = type === 'hotel' ? '🏨 Hotel' : '🍽️  Restaurante';
    const subject = `${typeLabel} Aguardando Aprovação: ${nome}`;

    const html = `<!DOCTYPE html>
<html lang="pt-BR">
<head><meta charset="utf-8"><title>Novo ${typeLabel}</title></head>
<body style="font-family:Arial,sans-serif;background:#f5f5f5;padding:40px 0;">
  <div style="max-width:600px;margin:0 auto;background:#fff;border-radius:8px;overflow:hidden;box-shadow:0 2px 8px rgba(0,0,0,0.1);">
    <div style="background:linear-gradient(135deg,#1d4ed8,#3b82f6);padding:32px;text-align:center;">
      <h1 style="margin:0;color:#fff;font-size:24px;">${typeLabel} Cadastrado</h1>
      <p style="margin:8px 0 0;color:#bfdbfe;font-size:14px;">Aguardando sua aprovação</p>
    </div>
    <div style="padding:32px;">
      <p style="font-size:15px;color:#374151;">Um novo ${type === 'hotel' ? 'hotel' : 'restaurante'} se cadastrou no <strong>Explore Aparecida</strong> e está aguardando aprovação.</p>
      <table style="width:100%;background:#f8fafc;border-radius:6px;border-left:4px solid #3b82f6;margin:20px 0;">
        <tr><td style="padding:20px;">
          <p style="margin:0 0 8px;color:#6b7280;font-size:13px;"><strong>Nome:</strong></p>
          <p style="margin:0 0 16px;font-size:16px;color:#111827;font-weight:600;">${nome}</p>
    ...
    const text = `${typeLabel} — Explore Aparecida\n\nUm novo ${type === 'hotel' ? 'hotel' : 'restaurante'} aguarda aprovação:\n\nNome: ${nome}\nWhatsApp: ${whatsapp}\n${email ? `E-mail: ${email}\n` : ''}Plano: ${planoBadge}\n\nData: ${now}`;

    return sendEmail({ to: adminEmail, subject, html, text });
}

/**
 * Envia e-mail ao proprietário informando que o perfil está em análise.
 *
 * @param {Object} params
 * @param {string} params.type   - 'hotel' ou 'restaurante'
 * @param {string} params.nome   - Nome do estabelecimento
 * @param {string} params.email  - E-mail do proprietário
 */
export async function sendBusinessAnalisisEmail({ type, nome, email }) {
    if (!email) {
        console.warn(`⚠️  [SES] E-mail do ${type} não informado — e-mail de análise ignorado.`);
        return { success: false, error: `E-mail do ${type} não informado` };
    }

    const typeLabel = type === 'hotel' ? 'hotel' : 'restaurante';
    const subject = `Seu perfil está em análise — Explore Aparecida`;
...
<body style="font-family:Arial,sans-serif;background:#f5f5f5;padding:40px 0;">
  <div style="max-width:600px;margin:0 auto;background:#fff;border-radius:8px;overflow:hidden;box-shadow:0 2px 8px rgba(0,0,0,0.1);">
    ...
    <div style="padding:32px;">
      <p style="font-size:16px;color:#374151;">Olá, <strong>${nome}</strong>!</p>
      <p style="font-size:15px;color:#374151;line-height:1.6;">
        Recebemos seu cadastro como ${typeLabel} no <strong>Explore Aparecida</strong>.
        Seu perfil está sendo analisado pela nossa equipe e em breve estará visível na plataforma.
      </p>
      <div style="background:#eff6ff;border-left:4px solid #3b82f6;border-radius:4px;padding:16px 20px;margin:24px 0;">
        <p style="margin:0;font-size:14px;color:#1d4ed8;font-weight:600;">⏳ Prazo de análise: até 24 horas</p>
    ...
    const text = `Olá, ${nome}!\n\nRecebemos seu cadastro como ${typeLabel} no Explore Aparecida.\nSeu perfil está em análise e será aprovado em até 24 horas.\n\nObrigado!\nEquipe Explore Aparecida`;
